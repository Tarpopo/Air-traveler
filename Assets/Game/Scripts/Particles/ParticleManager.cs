...
public class ParticleManager : MySingleton<ParticleManager>
{
    ...
    public void PlayParticle(ParticleTypes particleType,Vector3 position,Vector3 rotation,Vector3 scale,Transform target,float delay=1f)
    {
        var particleSettings = _particles.FirstOrDefault(item => item.ParticleType == particleType);
        var particle = _managerPool.Spawn<Particle>(PoolType.Fx,particleSettings.Prefab,position,Quaternion.Euler(rotation));
